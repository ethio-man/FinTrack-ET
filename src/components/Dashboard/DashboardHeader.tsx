  ...
  const [notifications, setNotifications] = useState([
    {
      ...
      desc: isAmharic
        ? "ከደንበኛ ዮሐንስ ቁጥር 1,450 ብር በቴሌብር ገብቷል።"
        : "1,450 ETB transferred successfully from customer Johanes.",
    ...
    },
    {
      ...
      desc: isAmharic
        ? "የእርስዎ የክሬዲት ነጥብ ስኬት ወደ A+ አድጓል። የባንክ አበዳሪዎች መገምገም ይችላሉ።"
        : "FinTrack Scoring Model evaluated merchant tier: A+ Premium Status.",
    ...
    },
    {
      ...
      desc: isAmharic
        ? "ሁሉም የሽያጭ እና ወጪ መዛግብት ወደ ደመና የአገልጋይ ምትኬ ተደርገዋል።"
        : "Automated ledger backups successfully synchronized with cloud servers.",
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            onClick={() =>
              alert(
                isAmharic
                  ? "የዳሽቦርድ መቆጣጠሪያ ፓነል"
                  : "FinTrack system dashboard menu is optimized for single screen experience."
              )
            }
          >
            <svg
          ...
          >
            ...
            >
              <path
                fill="#0077C5"
                d="M18 36c9.9411 0 18-8.0589 18-18 0-9.94114-8.0589-18-18-18C8.05886 0 0 8.05886 0 18c0 9.9411 8.05886 18 18 18Z"
              />
              <path
                fill="white"
                d="M23 11H13a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V13a2 2 0 00-2-2zm-1 8h-3v3h-2v-3h-3v-2h3v-3h2v3h3v2z"
              />
            </svg>
            <div className="flex flex-col text-left leading-none font-sans">
              <span className="text-[7px] font-black tracking-[0.16em] text-[#0077C5] uppercase">
                FINTRACK
              </span>
              <span className="text-xs font-black text-[var(--text-core)] tracking-tight leading-none mt-0.5">
                et
              </span>
            </div>
          </a>
        ...
        <div className="docs-search flex-1 max-w-md relative hidden md:block">
          ...
          {searchFocused && (
            <div className="absolute top-[calc(100%+6px)] left-0 right-0 bg-[var(--bg-panel)] border border-[var(--border-core)] shadow-xl rounded-lg p-2.5 text-xs text-left z-[1000] animate-fade-in space-y-1.5">
              <span className="block text-[10px] uppercase font-mono tracking-wider font-extrabold text-[var(--text-mute)] px-2 mb-1">
              ...
              >
                ...
                <span className="text-[10px] text-[var(--text-sec)] font-mono">
                  /sale
                </span>
              </button>
              <button
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="relative hidden md:block">
            <button
              ...
              title={`${
                isAmharic ? "Switch to English" : "ወደ አማርኛ ለመቀየር ጠቅ ያድርጉ"
              }`}
            >
          ...
          {/* INTERACTIVE SYNC STATUS INDICATOR */}
          <div className="relative hidden md:block">
            <button
              onClick={() => {
            ...
            {syncOpen && (
              <div className="absolute top-[calc(100%+8px)] right-0 w-64 bg-[var(--bg-panel)] border border-[var(--border-core)] rounded-xl shadow-2xl p-4 z-[2000] text-left animate-fade-in space-y-3">
                <div className="pb-2 border-b border-[var(--border-subtle)]">
                  <span className="text-[10px] font-black uppercase tracking-widest text-[var(--text-mute)] block mb-0.5">
                    {t.syncStatus}
                  </span>
                  <span className="text-xs font-bold text-[var(--text-core)]">
                    {isAmharic ? "ሁኔታውን ይምረጡ" : "Simulate Connection State"}
                  </span>
                </div>
                ...
                <div className="space-y-1">
                  <button
                    ...
                    className={`w-full text-left p-1.5 rounded-lg flex items-center justify-between text-xs transition-colors ${
                      syncState === "online"
                        ? "bg-green-500/10 text-green-500 font-extrabold"
                        : "hover:bg-[var(--bg-core)]"
                    }`}
                  >
                ...
                <p className="text-[10px] text-[var(--text-sec)] leading-relaxed italic border-t border-[var(--border-subtle)] pt-2.5">
                  {syncState === "offline"
                    ? isAmharic
                      ? "መተግበሪያው ከመስመር ውጭ ሲሆን ግብይቶችን በቁልፍ ያከማቻል፣ ሲገናኝ በቀጥታ ያመሳስላል።"
                      : " Ledger has 4 local offline sales queued. Reconnect simulation to flush."
                    : isAmharic
                    ? "መረጃው በሰዓቱ ከቴሌብርና ሲቢኢ (Telebirr/CBE) ጋር ተመሳስሏል።"
                    : "Active merchant channel linked to centralized cloud accounting nodes."}
                </p>
              </div>
            )}
          </div>
              ...
              <div className="absolute top-[calc(100%+8px)] right-0 w-80 bg-[var(--bg-panel)] border border-[var(--border-core)] rounded-xl shadow-2xl overflow-hidden z-[2000] text-left animate-fade-in">
                ...
                <div className="divide-y divide-[var(--border-subtle)] max-h-72 overflow-y-auto">
                  {notifications.map((n) => (
                    <div
                      key={n.id}
                      className={`p-3.5 text-xs transition-colors hover:bg-[var(--bg-core)] ${
                        !n.read ? "bg-[#0077C5]/5 border-l-2 border-[#0077C5]" : ""
                      }`}
                    >
                      <div className="flex items-center justify-between gap-1 mb-1">
                        <span className="font-extrabold text-[var(--text-core)]">
                          {n.title}
                        </span>
                        <span className="text-[9px] font-mono text-[var(--text-mute)]">
                          {n.time}
                        </span>
                      </div>
                      <p className="text-[var(--text-sec)] text-[11px] leading-relaxed select-text">
                        {n.desc}
                      </p>
                    </div>
                  ))}
                ...
                <div className="space-y-2 text-xs">
                  <div className="bg-[var(--bg-core)] p-2.5 border border-[var(--border-subtle)] rounded-lg">
                    <span className="font-extrabold block mb-0.5">
                      {isAmharic ? "📞 የስልክ ድጋፍ መስመር" : "📞 Domestic Hotline"}
                    </span>
                    <a
                      href="tel:9312"
                      className="text-[#0077C5] font-black text-sm block"
                    >
                      9312 (Toll-Free / ነጻ)
                    </a>
                    <span className="text-[10px] text-[var(--text-sec)] block mt-0.5">
                      {isAmharic
                        ? "ከሰኞ እስከ ቅዳሜ ከቀኑ 2፡00 - 12፡00 ሰዓት"
                  ...
                  <div className="space-y-1 pl-1">
                    <a
                      ...
                      onClick={(e) => {
                        ...
                        alert(
                          isAmharic
                            ? "የቴሌብር (Telebirr App SDK) ማስተካከያ ቪዲዮ መጫኛ..."
                            : "Loading SDK integrations tutorial..."
                        );
                      }}
                    ...
                    <a
                      ...
                      onClick={(e) => {
                        ...
                        alert(
                          isAmharic
                            ? "ለመጀመሪያ ጊዜ ተጠቃሚዎች የቪዲዮ መመሪያ ማሳያ..."
                            : "Opening merchant onboarding manual..."
                        );
                      }}
                      className="text-[#0077C5] hover:underline block font-semibold"
                    >
            ...
            {avatarOpen && (
              <div className="absolute top-[calc(100%+8px)] right-0 w-64 bg-[var(--bg-panel)] border border-[var(--border-core)] rounded-xl shadow-2xl z-[2000] text-left overflow-hidden animate-fade-in select-none">
                <div className="p-4 bg-[var(--bg-panel-inner)] border-b border-[var(--border-subtle)] space-y-1">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-[#0077C5] block">
                    {t.placeholderUser}
                  </span>
                  <span className="text-xs font-black text-[var(--text-core)] block tracking-tight truncate select-text">
                    {userEmail}
                  </span>
                  <span className="inline-flex text-[9px] font-medium bg-green-500/15 text-green-500 px-1.5 py-0.5 rounded uppercase font-mono mt-1">
                    Verified Merchant
                  </span>
                </div>
                  ...
                  >
                    <span className="flex items-center gap-2 font-semibold">
                      {isDarkMode ? (
                        ...
                        <Moon className="w-4 h-4 text-[#0077C5]" />
                      )}
                      {isDarkMode
                        ? isAmharic
                          ? "ብርሃን ገጽታ"
                    ...
                    <span className="text-[9px] font-bold tracking-widest text-[var(--text-mute)] uppercase">
